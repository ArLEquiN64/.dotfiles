...
let g:python_host_prog = system('(type asdf &>/dev/null && echo -n "$DOTPATH/asdf/installs/python/2.7.15/bin/python") || echo -n $(which python3)')
let g:python3_host_prog = system('(type asdf &>/dev/null && echo -n "$DOTPATH/asdf/installs/python/$(asdf current python | cut -d " " -f 1)/bin/python") || echo -n $(which python2)')
